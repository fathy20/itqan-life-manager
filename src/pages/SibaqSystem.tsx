...
export default function SibaqSystem() {
  ...
  const { todayScore, shared, weekScores } = useScore();
  const { halaqahs, leaderboard, duas, challenge, createHalaqah, joinHalaqah, addDua, prayForDua, sendNaseeha, completeChallenge } = useHalaqah();
  const activeHalaqah = halaqahs[0] || null;

  // Map live data to screen format
  const MY_SCORE_LIVE = {
    total: todayScore?.total || 0,
    rank: shared?.rankTitle || "مبتدئ",
    streak: shared?.streakDays || 0,
    weeklyRing: shared?.weeklyRingPercent || 0,
    weekScores: weekScores.map(s => s.total || 0),
    breakdown: [
      { label: "Salah", v: todayScore?.salah || 0, max: 30, color: "#A78BFA" },
      { label: "Quran", v: todayScore?.quran || 0, max: 20, color: "#34D399" },
      { label: "Adhkar", v: todayScore?.adhkar || 0, max: 15, color: "#FBBF24" },
      { label: "Productivity", v: todayScore?.productivity || 0, max: 15, color: "#60A5FA" },
      { label: "Health", v: todayScore?.health || 0, max: 10, color: "#F472B6" },
      { label: "Bonus", v: todayScore?.bonus || 0, max: 10, color: "#FB923C" },
    ],
  };
  const MEMBERS_LIVE = leaderboard.length > 0 ? leaderboard : MEMBERS;
  const DUAS_LIVE = duas.length > 0 ? duas : DUAS;
  const CHALLENGE_LIVE = challenge || CHALLENGE;
  const [naseehaOpen, setNaseehaOpen] = useState(false);
  const [duaInput, setDuaInput] = useState("");

  const inviteCode = "XK7M2P";
  const handleCopy = () => { setCopied(true); setTimeout(() => setCopied(false), 2000); };
  const rankData = RANKS[MY_SCORE_LIVE.rank] || RANKS["مبتدئ"];
