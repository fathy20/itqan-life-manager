...
const KHATMA = { currentPage: 390, totalPages: 604, dailyTarget: 4, todayRead: 2, daysLeft: 11, plan: "30-day" };
...
const HIFZ = [
  { name: "الفاتحة", num: 1, ayahs: 7, mem: 7, status: "memorized", review: null },
  { name: "البقرة", num: 2, ayahs: 286, mem: 120, status: "in_progress", review: null },
  { name: "يس", num: 36, ayahs: 83, mem: 83, status: "memorized", review: "today" },
  { name: "الملك", num: 67, ayahs: 30, mem: 30, status: "needs_review", review: "today" },
  { name: "الرحمن", num: 55, ayahs: 78, mem: 78, status: "memorized", review: "2d" },
  { name: "الواقعة", num: 56, ayahs: 96, mem: 40, status: "in_progress", review: null },
  { name: "النبأ", num: 78, ayahs: 40, mem: 40, status: "memorized", review: "1d" },
  { name: "الكهف", num: 18, ayahs: 110, mem: 0, status: "not_started", review: null },
];
...
export default function QuranSystem() {
  ...
  const totalMem = HIFZ.filter(s => s.status === "memorized").length;
  const reviewsDue = HIFZ.filter(s => s.review === "today").length;
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: TEXT, fontFamily: "'Inter', system-ui, sans-serif", padding: "24px 32px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14, marginBottom: 20 }}>
        ...
        <Card glow={reviewsDue > 0 ? "#F87171" : GREEN}>
          <div style={{ fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: "1.5px", marginBottom: 8 }}>Reviews due</div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
            <span style={{ fontSize: 34, fontWeight: 700, fontFamily: mono, color: reviewsDue > 0 ? "#F87171" : GREEN }}>{reviewsDue}</span>
          </div>
          <div style={{ fontSize: 11, color: MUTED, marginTop: 2 }}>{reviewsDue > 0 ? "Surahs need review today" : "All caught up!"}</div>
        </Card>
      </div>
